...
class Parser
{
     ...
     * @param \Repack\ScssPhp\Cache $cache
     */
     ...
     * @param string $msg
     *
     * @throws \Repack\ScssPhp\Exception\ParserException
     */
    ...
    protected function parseChunk()
    {
        ...
        if (isset($this->buffer[$this->count]) && $this->buffer[$this->count] === '@') {
            ...
            if ($this->literal('@include', 8) &&
                ...
                $child = array(Type::T_INCLUDE, $mixinName, isset($argValues) ? $argValues : null, null, isset($argUsing) ? $argUsing : null);
            ...
            if ($this->literal('@content', 8) &&
                ($this->end() ||
                    ...
                    $this->matchChar(')') &&
                    $this->end())) {
                $this->append(array(Type::T_MIXIN_CONTENT, isset($argContent) ? $argContent : null), $s);
        ...
        while (preg_match(static::$whitePattern, $this->buffer, $m, null, $this->count)) {
            if (isset($m[1]) && empty($this->commentsSeen[$this->count])) {
                ...
                if (!$comment) {
                    ...
                    $this->appendComment(array(Type::T_COMMENT, $c));
                } else {
                    $comment[] = $c;
                    $this->appendComment(array(Type::T_COMMENT, array(Type::T_STRING, '', $comment)));
                }
    ...
    protected function supportsQuery(&$out)
    {
        ...
        if (($this->literal('not', 3) && ($not = true) || true) &&
            ...
            $this->valueList($value) &&
            $this->matchChar(')')) {
            $support = array(Type::T_STRING, '', array(array(Type::T_KEYWORD, ($not ? 'not ' : '') . '(')));
            $support[2][] = $property;
            $support[2][] = array(Type::T_KEYWORD, ': ');
            $support[2][] = $value;
            $support[2][] = array(Type::T_KEYWORD, ')');
        ...
        if ($this->matchChar('(') &&
            ...
            $this->matchChar(')')) {
            $parts[] = array(Type::T_STRING, '', array(array(Type::T_KEYWORD, '('), $subQuery, array(Type::T_KEYWORD, ')')));
            $s = $this->count;
        } else {
            $this->seek($s);
        }
        ...
        if ($this->literal('not', 3) &&
            $this->supportsQuery($subQuery)) {
            $parts[] = array(Type::T_STRING, '', array(array(Type::T_KEYWORD, 'not '), $subQuery));
            $s = $this->count;
        } else {
            $this->seek($s);
        }
        ...
        if ($this->literal('selector(', 9) &&
            ...
            $this->matchChar(')')) {
            $support = array(Type::T_STRING, '', array(array(Type::T_KEYWORD, 'selector(')));

            $selectorList = array(Type::T_LIST, '', array());
            foreach ($selector as $sc) {
                $compound = array(Type::T_STRING, '', array());
                foreach ($sc as $scp) {
                    if (is_array($scp)) {
                        $compound[2][] = $scp;
                    } else {
                        $compound[2][] = array(Type::T_KEYWORD, $scp);
                    }
                }
        ...
        if ($this->literal('and', 3) &&
            ...
            array_unshift($expressions[2], array(Type::T_STRING, '', $parts));
            $parts = array($expressions);
            $s = $this->count;
        } else {
            $this->seek($s);
        }
    ...
    protected function expression(&$out)
    {
        ...
        if ($this->matchChar('[')) {
            if ($this->parenExpression($out, $s, "]", array(Type::T_LIST, Type::T_KEYWORD))) {
                if ($out[0] !== Type::T_LIST && $out[0] !== Type::T_MAP) {
                    $out = array(Type::T_STRING, '', array('[', $out, ']'));
                }
     ...
     * @return boolean
     */
    protected function parenExpression(&$out, $s, $closingParen = ")", $allowedTypes = array(Type::T_LIST, Type::T_MAP))
    {
    ...
    protected function value(&$out)
    {
        ...
        if ($this->literal('U+', 2) && $this->match('([0-9A-F]+\?*)(-([0-9A-F]+))?', $m, false)) {
            $out = array(Type::T_KEYWORD, 'U+' . $m[0]);
            return true;
        }
    ...
    protected function color(&$out)
    {
        ...
        if ($this->match('(#([0-9a-f]+))', $m)) {
            ...
            $channels = $hasAlpha ? array(4, 3, 2, 1) : array(3, 2, 1);
        ...
        for (;;) {
            ...
            if ($char === ':') {
                ...
                if ($this->mixedKeyword($nameParts, true)) {
                    ...
                    if ($nameParts === array('not') || $nameParts === array('is') ||
                        $nameParts === array('has') || $nameParts === array('where') ||
                        $nameParts === array('nth-child') || $nameParts == array('nth-last-child') ||
                        $nameParts === array('nth-of-type') || $nameParts == array('nth-last-of-type')
                    ) {
                        if ($this->matchChar('(', true) &&
