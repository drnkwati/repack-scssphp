...
use Repack\ScssPhp\Base\Range;
use Repack\ScssPhp\Block;
use Repack\ScssPhp\Cache;
use Repack\ScssPhp\Colors;
use Repack\ScssPhp\Compiler\Environment;
use Repack\ScssPhp\Exception\CompilerException;
use Repack\ScssPhp\Formatter\OutputBlock;
use Repack\ScssPhp\Node;
use Repack\ScssPhp\Parser;
use Repack\ScssPhp\SourceMap\SourceMapGenerator;
use Repack\ScssPhp\Type;
use Repack\ScssPhp\Util;
...
class Compiler
{
    ...
    /**
     * @var string|\Repack\ScssPhp\Formatter
     */
    protected $formatter = 'Repack\ScssPhp\Formatter\Nested';

    protected $rootEnv;
    protected $rootBlock;

    /**
     * @var \Repack\ScssPhp\Compiler\Environment
     */
    protected $env;
    protected $scope;
    protected $storeEnv;
     ...
     *
     * @return \Repack\ScssPhp\Formatter\OutputBlock
     */
     ...
     * Compile root
     *
     * @param \Repack\ScssPhp\Block $rootBlock
     */
            ...
            if ($this->matchExtendsSingle($part, $origin)) {
                ...
                foreach ($origin as $new) {
                    ...
                    if (!empty($before) && count($new) > 1) {
                        $preSharedParts = $k > 0 ? array_slice($before, 0, $k) : array();
    ...
    protected function matchExtendsSingle($rawSingle, &$outOrigin)
    {
        ...
        if (in_array($rawSingle, array(array('>'), array('+'), array('~')))) {
            return false;
        }
     ...
     * Compile media
     *
     * @param \Repack\ScssPhp\Block $media
     */
     ...
     * Media parent
     *
     * @param \Repack\ScssPhp\Formatter\OutputBlock $scope
     *
     * @return \Repack\ScssPhp\Formatter\OutputBlock
     */
     ...
     * Compile directive
     *
     * @param \Repack\ScssPhp\Block $block
     */
     ...
     *
     * @param \Repack\ScssPhp\Formatter\OutputBlock $scope
     * @param \Repack\ScssPhp\Formatter\OutputBlock $previousScope
     *
     * @return mixed
     */
    ...
    protected function compileWith($withCondition)
    {
        ...
        $with = array();
        $without = array('rule' => true);

        if ($withCondition) {
            if ($this->libMapHasKey(array($withCondition, static::$with))) {
                $without = array(); // cancel the default
                $list = $this->coerceList($this->libMapGet(array($withCondition, static::$with)));
            ...
            if ($this->libMapHasKey(array($withCondition, static::$without))) {
                $without = array(); // cancel the default
                $list = $this->coerceList($this->libMapGet(array($withCondition, static::$without)));
     ...
     * @param array $without
     *
     * @return \Repack\ScssPhp\Compiler\Environment
     */
     ...
     * Filter WITH rules
     *
     * @param \Repack\ScssPhp\Block|\Repack\ScssPhp\Formatter\OutputBlock $block
     * @param array                                                         $with
     * @param array                                                         $without
     *
     * @return boolean
    ...
    protected function isWith($block, $with, $without)
    {
        if (isset($block->type)) {
            ...
            if (count($block->selectors)) {
                ...
                if (is_object($s) && get_class($s) === 'Repack\ScssPhp\Node\Number') {
    ...
    protected function collapseSelectors($selectors, $selectorFormat = false)
    {
        ...
        foreach ($selectors as $selector) {
            ...
            if ($selectorFormat) {
                foreach ($output as &$o) {
                    $o = array(Type::T_STRING, '', array($o));
                }
                $output = array(Type::T_LIST, ' ', $output);
            } else {
     ...
     *
     * @param array                                  $stms
     * @param \Repack\ScssPhp\Formatter\OutputBlock $out
     ...
     * @param array                                  $stms
     * @param \Repack\ScssPhp\Formatter\OutputBlock $out
     * @param \Repack\ScssPhp\Block                 $selfParent
     * @param string                                 $traceName
     *
     * @throws \Exception
     */
    protected function compileChildrenNoReturn($stms, OutputBlock $out, $selfParent = null, $traceName = '')
    {
        ...
        foreach ($stms as $stm) {
            ...
            } elseif ($selfParent && in_array($stm[0], array(TYPE::T_INCLUDE, TYPE::T_EXTEND))) {
        ...
        foreach ($queryList as $kql => $query) {
            ...
            foreach ($query as $kq => $q) {
                for ($i = 1; $i < count($q); $i++) {
                    ...
                    $queryList[$kql][$kq][$i] = array(Type::T_KEYWORD, $value);
                }
            }
            if ($shouldReparse) {
                if (is_null($parser)) {
                ...
                $queryString = $this->compileMediaQuery(array($queryList[$kql]));
            ...
            foreach ($query as $q) {
                switch ($q[0]) {
                    case Type::T_MEDIA_TYPE:
                        $newType = array_map(array($this, 'compileValue'), array_slice($q, 1));
    ...
    protected function mergeDirectRelationships($selectors1, $selectors2)
    {
        ...
        do {
            ...
            if (!$this->isImmediateRelationshipCombinator($part1[0]) && $part1 !== $part2) {
                ...
                $isImmediateRel = $this->isImmediateRelationshipCombinator(reset($merged));

                if ($isImmediateRel[0]) {
                    array_unshift($merged, array($part1[0] . $part2[0]));
     ...
     *
     * @param string                                        $line
     * @param @param \Repack\ScssPhp\Formatter\OutputBlock $out
     * @param array                                         $allowed
     */
    protected function appendRootDirective($line, $out, $allowed = array(Type::T_COMMENT))
    {
        $root = $out;
     ...
     * @param boolean $inExp
     *
     * @return array|\Repack\ScssPhp\Node\Number
     */
    ...
    public function normalizeValue($value)
    {
        ...
        switch ($value[0]) {
            ...
            case Type::T_STRING:
                return array($value[0], '"', array($this->compileStringContent($value)));
     ...
     * @param array $right
     *
     * @return \Repack\ScssPhp\Node\Number
     */
     ...
     *
     * @param \Repack\ScssPhp\Compiler\Environment $env
     * @param \Repack\ScssPhp\Block                $selfParent
     *
     * @return array
     */
    ...
    protected function multiplyMedia(Environment $env = null, $childQueries = null)
    {
        ...
        $store = array($this->env, $this->storeEnv);
        $this->env = $env;
        $this->storeEnv = null;
            ...
            foreach ($parentQueries as $parentQuery) {
                foreach ($originalQueries as $childQuery) {
                    $childQueries[] = array_merge(
                        $parentQuery,
                        array(array(Type::T_MEDIA_TYPE, array(Type::T_KEYWORD, 'all'))),
                        $childQuery
                    );
                }
            }
     ...
     *
     * @return \Repack\ScssPhp\Compiler
     */
     ...
     *
     * @throws \Repack\ScssPhp\Exception\CompilerException
     */
        ...
        foreach ($prototypes as $prototype) {
            ...
            foreach ($prototype as $i => $p) {
                ...
                if (count($p)) {
                    ...
                    if ($p === 'null') {
                        ...
                        $default = array(Type::T_KEYWORD, 'null');
                    } else {
                        if (is_null($parser)) {
                            $parser = $this->parserFactory(__METHOD__);
                        }
                ...
                $argDef[] = array($name, $default, $isVariable);
            }
            ...
            try {
                ...
                foreach ($prototype as $i => $p) {
                    ...
                    if ($finalArgs[$i] === array(Type::T_KEYWORD, 'null')) {
                        $finalArgs[$i] = null;
                    }
                }
    ...
    protected function applyArguments($argDef, $argValues, $storeInEnv = true, $reduce = true)
    {
        ...
        foreach ($args as $arg) {
            ...
            if ($isVariable) {
                $val = array(Type::T_LIST, is_null($splatSeparator) ? ',' : $splatSeparator, array(), $isVariable);
        ...
        if (isset($item) && $item[0] === Type::T_MAP) {
            ...
            for ($i = 0, $s = count($keys); $i < $s; $i++) {
                ...
                switch ($key[0]) {
                    ...
                    default:
                        $key = array(Type::T_KEYWORD, $this->compileStringContent($this->coerceString($key)));
                        break;
                }
    ...
    protected function libCall($args, $kwargs)
    {
        ...
        foreach ($kwargs['args'][2] as $varname => $arg) {
            if (is_numeric($varname)) {
                ...
                $varname = array('var', $varname);
            }

            $callArgs[] = array($varname, $arg, false);
        }

        return $this->reduce(array(Type::T_FUNCTION_CALL, $name, $callArgs));
    }
    ...
    // mix two colors
    protected static $libMix = array('color-1', 'color-2', 'weight:0.5');
    ...
    protected static $libSaturate = array(array('color', 'amount'), array('number'));
    ...
    protected static $libJoin = array('list1', 'list2', 'separator:null');
    ...
    protected static $libAppend = array('list', 'val', 'separator:null');
    ...
    protected static $libStrSlice = array('string', 'start-at', 'end-at:null');
    ...
    protected static $libIsSuperselector = array('super', 'sub');
    ...
    protected static $libSelectorAppend = array('selector...');
    protected function libSelectorAppend($args)
    {
        ...
        $selectors = array_map(array($this, 'getSelectorArg'), $args);
    ...
    protected static $libSelectorExtend = array('selectors', 'extendee', 'extender');
    ...
    protected static $libSelectorReplace = array('selectors', 'original', 'replacement');
    ...
    protected static $libSelectorNest = array('selector...');
    protected function libSelectorNest($args)
    {
        ...
        $selectorsMap = array_map(array($this, 'getSelectorArg'), $args);
    ...
    protected static $libSelectorParse = array('selectors');
    ...
    protected static $libSelectorUnify = array('selectors1', 'selectors2');
    ...
    protected function unifyCompoundSelectors($compound1, $compound2)
    {
        ...
        $unifiedCompound = array($last);
        $unifiedSelectors = array($unifiedCompound);
        ...
        while (count($compound1) || count($compound2)) {
            ...
            if ($part1 && ($match2 = $this->matchPartInCompound($part1, $compound2))) {
                ...
                $unifiedSelectors = $this->prependSelectors($unifiedSelectors, array($c));
                $part1 = $part2 = null;
            ...
            if ($part1 && $part2) {
                ...
                $s = $this->prependSelectors($unifiedSelectors, array($part2));
                $new = array_merge($new, $this->prependSelectors($s, array($part1)));
                $s = $this->prependSelectors($unifiedSelectors, array($part1));
                $new = array_merge($new, $this->prependSelectors($s, array($part2)));
            } elseif ($part1) {
                array_pop($compound1);

                $new = array_merge($new, $this->prependSelectors($unifiedSelectors, array($part1)));
            } elseif ($part2) {
                array_pop($compound2);

                $new = array_merge($new, $this->prependSelectors($unifiedSelectors, array($part2)));
            }
    ...
    protected function mergeParts($parts1, $parts2)
    {
        ...
        if ($tag) {
            if ($tag1) {
                $parts1 = array_diff($parts1, array($tag1));
            }

            if ($tag2) {
                $parts2 = array_diff($parts2, array($tag2));
            }
        }

        $mergedParts = array_merge($parts1, $parts2);
        $mergedOrderedParts = array();
    ...
    protected static $libSimpleSelectors = array('selector');
    protected function libSimpleSelectors($args)
    {
        ...
        foreach ($part as $p) {
            $listParts[] = array(Type::T_STRING, '', array($p));
        }

        return array(Type::T_LIST, ',', $listParts);
    }
}
